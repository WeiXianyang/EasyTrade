...
| 加分项 | 完成情况 |
|--------|----------|
| 后端联动 | 已接入独立 Spring Boot 真实后端 API，前台商品、购物车、订单、登录和后台管理均通过 Axios 调用 `/easytrade/api`。 |
| 数据持久化 | 业务数据持久化到 PostgreSQL；前端通过 `localStorage` 保存 JWT 与脱敏用户快照，刷新后登录态不丢失。 |
| 表单验证 | 登录、注册、短信验证码、收货地址、后台商品新增/编辑和分类表单均配置必填、格式或业务校验。 |
| 分页/无限滚动 | 前台分类商品列表使用分页；订单列表、后台商品、分类和订单表格使用分页或横向滚动。 |
| 支付模拟优化 | 支付页提供步骤条、60 秒支付倒计时、进度条和支付成功结果页。 |
| 响应式布局 | 前台商品网格、底部导航、悬浮客服/购物车入口和后台表格适配手机、平板与 PC。 |
| 性能优化 | 路由页面使用 `lazy + Suspense` 懒加载，商品卡片使用 `React.memo`，购物车和后台数据刷新等交互使用 `useCallback` 稳定回调。 |
| 单元测试 | 前端使用 Node 内置 test runner 覆盖 API 客户端、路由、服务、页面结构和工程规范；后端使用 JUnit 5 覆盖认证、商品、购物车、订单和后台运营服务。 |
| 部署上线 | 已部署到 [https://www.helvzn.com/easytrade/](https://www.helvzn.com/easytrade/)，生产 API 可通过 `/easytrade/api` 访问。 |
...
线上地址：[https://www.helvzn.com/easytrade/](https://www.helvzn.com/easytrade/)
